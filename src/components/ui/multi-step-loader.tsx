      ...
      {loadingStates.map((loadingState, index) => {
        ...
        return (
          ...
          >
            <div>
              {index > value && (
                <CheckIcon className="text-white dark:text-black" />
              )}
              {index <= value && (
                <CheckFilled
                  className={cn(
    ...
    <AnimatePresence mode="wait">
      {loading && (
        ...
        >
          ...
          <div className="bg-gradient-to-t inset-x-0 z-20 bottom-0 bg-black dark:bg-white h-full absolute [mask-image:radial-gradient(900px_at_center,transparent_30%,black)]" />
        </motion.div>
      )}
    </AnimatePresence>
  );
